...
public class UserAbsenteeismController : ControllerBase
{
    ...
    {
        return Ok(await _userAbsenteeismService.GetAllAsync(x => x.EndDate >= DateTime.UtcNow.AddDays(-365), includeProperties: "User,AbsenteeismType").ConfigureAwait(false));
    }
    ...
    {
        return Ok(await _userAbsenteeismService.GetAllAsync(x => x.UserId == id && x.EndDate >= DateTime.UtcNow.AddDays(-365), includeProperties: "User,AbsenteeismType").ConfigureAwait(false));
    }
